...
注意 `pwsh -Command "& sentinel.ps1 ..."` 会把任何非零码压成 `1`。要拿到精确退出码请用 `pwsh -File`，或在末尾加 `; exit $LASTEXITCODE`。
...
任务执行期间，`GET /tasks/<id>` 会附带 `progress` 字段，`wait` 也会在新内容出现时逐行打印。agent 可以往逐任务的进度文件里追加内容——刚拿到的 job ID、正在做哪一步、卡在哪里——走的是和结果完全相同的通道。这是**可选的**：什么都不报的 agent 并没有做错任何事，进度是给运维的便利，不是第二份可以让任务失败的契约。任务进入终态时该文件会被删除，中途被杀的任务留下的则由启动清扫收走。

进度走文件通道而不是终端，是有意的选择：终端读取会截断、混入 TUI 渲染字符，而且实测出现过把输入预测当成进行中工作的误判。
...
每条委派任务都带一个 Slurm 策略，在 agent 动手之前写进 prompt。可按请求用 `slurm_policy` 指定，或用 `SENTINEL_SLURM_POLICY` 改部署默认值。
...
无法识别的策略会被拒绝，而不是悄悄替换成默认值——否则把最严格那档拼错，结果会变成部署允许的最宽松那档。任务按入队时记录的策略执行，而不是按 worker 取到它时的默认值。

bridge 从不自行重跑任务。被重启打断的任务会变成 `orphaned` 并停在那里，所以不明的 Slurm 状态绝不会靠重新提交来"解决"。
